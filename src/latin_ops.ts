...
export function cipher_encode(L: List<number>): List<number> {
    return (L === nil ? nil : cons(next_latin_char(L.hd), cipher_encode(L.tl)));
}

export function cipher_decode(L: List<number>): List<number> {
    return (L === nil ? nil : cons(prev_latin_char(L.hd), cipher_decode(L.tl)));
}

export function crazy_caps_encode(L: List<number>): List<number> {
    return (L === nil ? nil : cons(String.fromCharCode(L.hd).toUpperCase().charCodeAt(0), crazy_caps_encode_odd(L.tl)));
}

export function crazy_caps_encode_odd(L: List<number>): List<number> {
    return (L === nil ? nil : cons(L.hd, crazy_caps_encode(L.tl)));
}

export function crazy_caps_decode(L: List<number>): List<number> {
    return (L === nil ? nil : cons(String.fromCharCode(L.hd).toLowerCase().charCodeAt(0), crazy_caps_decode_odd(L.tl)));
}

export function crazy_caps_decode_odd(L: List<number>): List<number> {
    return (L === nil ? nil : cons(L.hd, crazy_caps_decode(L.tl)));
}